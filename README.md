...
CREATE TABLE vehicles (
  ...
  first_seen_date TIMESTAMP WITH TIME ZONE DEFAULT timezone('US/Eastern', now()),
  last_seen_date TIMESTAMP WITH TIME ZONE DEFAULT timezone('US/Eastern', now())
);
...
CREATE TABLE parking_sessions (
  ...
  entry_time TIMESTAMP WITH TIME ZONE DEFAULT timezone('US/Eastern', now()),
  exit_time TIMESTAMP WITH TIME ZONE,
  lot_id TEXT REFERENCES parking_lots(lot_id),
...
CREATE TABLE parking_lots (
  ...
  location JSONB,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('US/Eastern', now())
);

-- Create a view for real-time occupancy tracking
CREATE VIEW parking_lot_occupancy AS
SELECT 
  pl.lot_id, 
  pl.name, 
  pl.max_capacity, 
  pl.location, 
  pl.created_at, 
  COUNT(ps.session_id) AS current_occupancy
FROM parking_lots pl
LEFT JOIN parking_sessions ps ON pl.lot_id = ps.lot_id AND ps.status = 'active'
GROUP BY pl.lot_id;
```
